...
//emwin main page	
    ...
    hWin = ICONVIEW_CreateEx(50,40,750,440,WM_HBKWIN,WM_CF_SHOW|WM_CF_HASTRANS,0,GUI_ID_ICONVIEW0,200,120+60);
	for(i = 0; i<GUI_COUNTOF(_bitmapItem); i++)
	{
		ICONVIEW_AddBitmapItem(hWin, _bitmapItem[i].pBitmap, _bitmapItem[i].pText);
        ICONVIEW_SetTextColor(hWin, ICONVIEW_CI_UNSEL, GUI_WHITE );
        ICONVIEW_SetTextColor(hWin, ICONVIEW_CI_SEL, GUI_RED );
		ICONVIEW_SetBkColor(hWin, ICONVIEW_CI_SEL, GUI_BLACK);
	}
	ICONVIEW_SetSel(hWin, 0);
	ICONVIEW_SetFont(hWin, &BM_Font_mainpagecontent);
	ICONVIEW_SetTextAlign(hWin, GUI_TA_HCENTER|GUI_TA_BOTTOM);

	ICONVIEW_SetIconAlign(hWin, ICONVIEW_IA_HCENTER|ICONVIEW_IA_TOP);
	ICONVIEW_SetSpace(hWin, GUI_COORD_X, 50);
	ICONVIEW_SetSpace(hWin, GUI_COORD_Y, 40);
	WM_SetCallback(WM_HBKWIN, _cbHBKWindow);
	WM_SetFocus(hWin);
	sysprintf("start main window with iconview\n");
    while (1)
    {
        GUI_Delay(50);
    }
